...
**Note:** LEAD inherited from the brief — a genuine oil / US-Iran risk-off where fear finally showed up. Hook carries the consequential frame: a third down day where the VIX rose on a red day for the first time in the slide, not just another drift. Material after-close development + forward in the MAIN post per check 18: IndusInd Bank reported after the close (reaction lands Thursday), plus Infosys on Thursday and the 24-Jul US tariff cliff on Friday. India-specific decoupling kept in-post (world rose, India fell on its own crude bill). Day-names, no today/tomorrow; no em-dash in body (firebreak only); no emojis; no thread tropes; no recommendation language. Long-form needs X Premium; a 280-char/thread fallback available on request. Zero stock-name overlap with the two replies (main names only IndusInd Bank + Infosys, plus indices/oil).
