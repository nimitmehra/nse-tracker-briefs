# Podcast Script — 2026-08-27 (evening recap: Thursday's session, and half the index fall was one bank)

**Word count:** 742 words (spoken body only)
**Estimated duration:** 4.50 minutes at 165 WPM
**Self-verify:** PASS with one declared overrun. Machine-checked on the spoken body: zero digits, zero currency or percent symbols, zero em-dashes or en-dashes, zero `[SAY:]` hints, zero exclamation points, zero banned trader jargon, zero recommendation language before the firebreak (the regex hit on "sell people's time" is a word-fragment false positive). Longest sentence 38 words, single-thread and comma-segmented, which the skill permits. All nine sections present, no start firebreak. **Word count 742, forty two over the 700 soft ceiling** — at 165 WPM that is 4.50 minutes, the top of the skill's own 3.5-4.5 minute duration band, so the episode is in time even though it is over the word band. The draft was cut from 1,346 words; nothing was spent on embellishment.
**Deliberate cuts (declared, not hidden):** the Sensex derivatives-expiry mechanics; the India VIX beat (up 4.71% to 11.07, agreeing with the index for the first time in three sessions but off a floor); the gold-lender cohort fall and its loan-to-value mechanism; the crude-did-not-transmit contrast; the institutional votes against promoters at four companies; the six-penalties-in-five-days regulatory pattern; the RBI reverse-repo self-correction; the Skipper / Transformers & Rectifiers / Viviana order-announcement trio; the empty earnings scan; and the Friday 28 August items (Jackson Hole, the RBI weekly statistics, the ₹34,000 crore bond sale, the NBFC comment deadline). The written brief carries all of them.
**Skill version:** podcast-script-public-nse v1.7 (evening recap model)
**TTS-ready:** YES
**Source:** `briefs/public/2026-08-27.md` only. No independent reporting, no web searches.
**Continuity:** the last episode was 2026-08-24 (Monday's session). Tuesday 25 and Wednesday 26 August have no brief and no episode, so they are unseen by the listener; the script dates anything from those two sessions to its own day, exactly as the brief does (the RBI bulletin as Tuesday's, the Bombay Burmah filing as Wednesday's).
**Time-words:** day names throughout, per the standing deviation established 2026-08-20 and carried by the 2026-08-24 episode. The covered session is "Thursday" and is spoken in the past tense. The one forward date named is Monday 31 August.
**"What to Watch" deviation, flagged:** the brief's top-listed catalysts are Friday 28 August items. The script instead points at **Monday 31 August**, which the brief itself calls "the week's real date." Reason: the Friday items are same-day or already past by any plausible publication time, and a stale forward-look is worse than a later one. This is a deliberate departure from "take the top item in What to Watch."
**No `[SAY:]` hints** — `tts-podcast-nse.py` strips them with no substitution, so they are inert. Mangle risks are respelled in the spoken body: **toro I-Q** for the brand, **H-D-F-C Bank**, **M-S-C-I**. The Reserve Bank of India is said in full, never as initials.
